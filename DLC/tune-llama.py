...
class KernelFlagsTuner(MeasurementInterface):
  ...
  def run_precompiled(self, desired_result, input, limit, compile_result, id):
    ...
    if self.is_executor:
      ...
      dlc_id = os.environ.get('CHIPLTECH_VISIBLE_DEVICES', os.environ.get('DLC_VISIBLE_DEVICES', '2'))
      run_cmd = "DLC_VISIBLE_DEVICES={0} ACCELERATE_TORCH_DEVICE=dlc python3 sft_trainer.py --device=dlc \
                --model=/mnt/jfs/ci_models/Llama-2-7b-hf --max_seq_length=256 --dtype=bfloat16 --dropout=0.05".format(dlc_id)
...
if __name__ == '__main__':
  ...
  with open('/mnt/jfs/ci-dingtalk/autotune/llama/cycles_data_llama.csv', 'a', newline='') as csvfile:
