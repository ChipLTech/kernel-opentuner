    ...
    if self.is_executor:
      ...
      with subprocess.Popen(run_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1, universal_newlines=True, shell=True) as p:
        for line in p.stdout:
            ...
            run_result += line
      if p.returncode:
        raise subprocess.CalledProcessError(p.returncode, run_cmd)
      print("Model run finished")
      
      # save the log
      subprocess.call("touch " + self.log_root + "gemma_iter" + str(iteration_count.value) + ".log", shell=True)
      with open(self.log_root + "gemma_iter" + str(iteration_count.value) + ".log", 'w') as f:
        f.write(run_result)
      print("Log saved")
      # with open('/home/test/lanhu/autotune/logs/1727544383/gemma_iter0.log') as f:
      #   run_result = f.read()
...
if __name__ == '__main__':
  ...
  with open('/home/CI/autotune/cycles_data_gemma.csv', 'a', newline='') as csvfile:
